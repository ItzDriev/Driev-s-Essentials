...
-- Which DR bracket each crowd-control spell shares. Two spells in the same
-- category diminish each other: full duration, then half, then a quarter, then
-- immune, with the bracket resetting ~18s after the last one falls off. The
-- engine applies that multiplier in Durations.lua; this file only says what
-- shares a bracket with what.
--
-- The commented-out entries are deliberate and worth leaving in place: several
-- categories people expect from later expansions did NOT exist in 1.12, and a
-- future reader who "fixes" the omission would make our timers wrong. Silences
-- weren't diminished until 3.0.8, Disarm until 2.3.0, Frostbite until 2.1.0,
-- and Blind sat with Fear only from 3.0 onward.
--
-- Data transcribed from LibClassicDurations (diminishingReturns.lua, DRTable
-- version 7) by d87 — https://github.com/rgd87/LibClassicDurations
...
-- Categories that also diminish against creatures. Everything else is
-- PvP-only: a mob chain-feared in a dungeon takes full duration every time.
D.drPvECategories = {
    [KIDNEY_SHOT] = true,
    [STUN] = true,
}
