...
-- ── Player spell durations ───────────────────────────────────────────────────
-- The rules for every player-castable aura in 1.12: how long it lasts, whether
-- rank changes that, whether a talent or set bonus extends it, whether it needs
-- a matching cast event to be believed, and whether two casters can stack their
-- own copies on one target.
--
-- `Spell(id, opts)` takes a single ID or a list of ranks that share one rule.
-- The opts that matter to the engine (Durations.lua):
--
--   duration     seconds, or a function(spellID, isSrcPlayer, comboPoints).
--                A function is how rank, talent, set bonus and combo-point
--                scaling get expressed; returning nil means "we cannot know",
--                and the engine shows no timer rather than a wrong one.
--   pvpduration  the cap used instead when the target is player-controlled.
--   stacking     two casters keep separate timers on the same target (DoTs).
--   castFilter   only believe this aura when we also saw the cast. Long raid
--                buffs need it: walking into combat log range of a player fires
--                AURA_APPLIED as if their hour-old Fortitude were fresh.
--   type/buffType  "BUFF" and the dispel school, for enemy buff synthesis.
--
-- Talent(...) returns the rank of the first talent spell the PLAYER knows, so
-- talent-scaled durations are only exact for our own casts — for anyone else
-- the untalented value is used, which is why so many of these functions branch
-- on isSrcPlayer.
--
-- Data transcribed from LibClassicDurations (classAbilities.lua, SpellTable
-- version 71) by d87 — https://github.com/rgd87/LibClassicDurations

local D = addon.Durations
if not D then return end

-- The body below is written against `lib`, matching the upstream data file it
-- came from, so re-syncing a future correction stays a diff of the data itself
-- rather than a rewrite of every line.
local lib   = D
local Spell = D.AddAura
local Talent = D.Talent

local INFINITY = math.huge
local _, class = UnitClass("player")
local locale   = GetLocale()

-- Classic Era still has the global, but prefer C_Spell where the client has it,
-- the same way Data.SpellInfo does in the nameplate module.
    ...
    for _, spellId in ipairs(fire_spells) do
        ...
        lib.indirectRefreshSpells[spellName][12654] = {
            ...
            customAction = function(srcGUID, dstGUID, spellID)
                -- `lib` is the file-scope upvalue above; no library lookup needed.
